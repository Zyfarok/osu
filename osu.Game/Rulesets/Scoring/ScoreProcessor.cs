...
namespace osu.Game.Rulesets.Scoring
{
    public partial class ScoreProcessor : JudgementProcessor
    {
        ...
        /// </summary>
        public readonly BindableLong BaseScoreSinceLastBreak = new BindableLong();
        ...
        /// <summary>
        /// The default portion of <see cref="max_score"/> that only accounts for hitting <see cref="HitObject"/>s accurately. Defaults to 30%.
        /// </summary>
        protected virtual double DefaultAccuracyPortion => 0.3;

        /// <summary>
        /// The default portion of <see cref="max_score"/> that also accounts for combos. Defaults to 70%.
        /// </summary>
        ...
        /// </summary>
        protected virtual double DefaultAccuracyExponent => 8;

        /// <summary>
        /// The default exponent applied to the accuracy ratio used in the combo portion of the score. Defaults to 2.
        /// </summary>
        protected virtual double DefaultComboAccuracyExponent => 2;
        ...
        private readonly double comboAccuracyExponent;
        ...
        public ScoreProcessor(Ruleset ruleset)
        {
            ...
            comboAccuracyExponent = DefaultComboAccuracyExponent;
            ...
            if (accuracyExponent <= 0)
                throw new InvalidOperationException($"{nameof(DefaultComboAccuracyExponent)} should be positive.");
        ...
        protected sealed override void ApplyResultInternal(JudgementResult result)
        {
            ...
            result.BaseScoreSinceLastBreak = BaseScoreSinceLastBreak.Value;
            ...
            if (!result.Type.IsBonus())
                BaseScoreSinceLastBreak.Value += result.Type.IsHit() ? Judgement.ToNumericResult(result.Type) : 0;
            ...
            else if (result.Type.BreaksCombo())
            {
                ...
                Combo.Value = 0;
                BaseScoreSinceLastBreak.Value = 0;
            }
            ...
            currentScoringValues.ComboScore =
                computeComboScore(ComboScoreAtLastBreak.Value, BaseScoreSinceLastBreak.Value, Combo.Value);
        ...
        protected sealed override void RevertResultInternal(JudgementResult result)
        {
            ...
            BaseScoreSinceLastBreak.Value = result.BaseScoreSinceLastBreak;
        ...
        {
            double accuracyScore = accuracyPortion * Math.Pow(accuracyRatio, accuracyExponent) * progress;
            double comboScore = comboPortion * Math.Pow(accuracyRatio, comboAccuracyExponent) * comboRatio;
        ...
        public virtual void PopulateScore(ScoreInfo score)
        {
            ...
            score.ComboScoreAtLastBreak = ComboScoreAtLastBreak.Value;
            score.BaseScoreSinceLastBreak = BaseScoreSinceLastBreak.Value;
            ...
            score.ComboScore = currentScoringValues.ComboScore;
        }
        ...
        public override void ResetFromReplayFrame(ReplayFrame frame)
        {
            ...
            BaseScoreSinceLastBreak.Value = frame.Header.BaseScoreSinceLastBreak;
            ComboScoreAtLastBreak.Value = frame.Header.ComboScoreAtLastBreak;
            currentScoringValues.ComboScore = computeComboScore(ComboScoreAtLastBreak.Value, BaseScoreSinceLastBreak.Value, Combo.Value);
        ...
        private void extractScoringValues(ScoreInfo scoreInfo, out ScoringValues current, out ScoringValues currentMaximum, out ScoringValues maximum)
        {
            ...
            current.ComboScore = scoreInfo.ComboScore;
